...
namespace KirasFM_Grid_Generator {
  ...
  // We approximate the refrective index between two points by the following distribution
  class refrective_index {
      public:
        refrective_index(const double s, const double r1, const double r2, const double a, const double b):
          r1(r1),
          r2(r2),
          a(a),
          b(b)
        ...
        double get_refrective_index(const unsigned int i, const double px, const double py) const {
          if( i != 1 ) {
            double out = r1;
            for(auto p : p_list) {
  ...
  // (return false)
  template<int dim>
  bool in_trapez(Point<dim> p , const double s = 1.0) {
  ...
  */
  ...
  void cylinder(
    ...
    GridGenerator::subdivided_hyper_rectangle(tria, repetitions, lower_left, upper_right);
    ...
    for ( unsigned int i = 0; i < 2; i++ ) {
      ...
      const std::vector< Point<dim> > cell = {
    ...
    for (auto &cell : tria.cell_iterators()) {
      for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; face++) {
        if( cell->face(face)->center()[2] == 0 || cell->face(face)->center()[2] == L )
          continue;
   ...
   */
  template<int dim>
  void hyper_cube_with_cylindrical_structure (
    Triangulation<dim>& tria,
    ...
    Tensor<1, dim> shift_vector1({lower_left(0) + outer_radius, lower_left(1) + outer_radius, lower_left(2)});
    Tensor<1, dim> shift_vector2({lower_left(0) + ( outer_radius /2.0), lower_left(1) + ( outer_radius / 2.0), lower_left(2)});

    { // create the shell
      Triangulation<dim> tmp_tria;
      GridGenerator::hyper_cube_with_cylindrical_hole(tmp_tria, inner_radius, outer_radius, L);
    ...
    Tensor<1, dim> direction({0.0, 0.0, 1.0});
    CylindricalManifold<dim> manifold( direction, Point<dim>(lower_left(0) + (outer_radius), lower_left(1) + (outer_radius), 0.0));
    tria.set_manifold(manifold_id, manifold );

  }


  // We split the creation of the Waveguide into three magic steps, and here follows the magic
  template<int dim>
  void magic1(
      Triangulation<dim> &tria,
      double L,
    ...
    { // second part:
      ...
      GridGenerator::subdivided_hyper_rectangle(tmp_tria, repetitions, lower_left, upper_right);
      GridGenerator::merge_triangulations(tria, tmp_tria, tria, 1e-10, true);
    }

    { // left hole: (manifold id = 0)
      Triangulation<dim> tmp_tria;
      hyper_cube_with_cylindrical_structure(tmp_tria, Point<3>(0.2 * s, lower, 0.0), 0.07 * s, 0.1 * s, L, 0);
      GridGenerator::merge_triangulations(tria, tmp_tria, tria, 1e-10, true);
    }
    ...
    { // right hole: (manifold id = 1)
      Triangulation<dim> tmp_tria;
      hyper_cube_with_cylindrical_structure(tmp_tria, Point<3>(0.6 * s, lower, 0.0), 0.07 * s, 0.1 * s, L, 1);
      GridGenerator::merge_triangulations(tria, tmp_tria, tria, 1e-10, true);
    }
    ...
    // set the manifold id:
    Tensor<1, dim> direction({0.0, 0.0, 1.0});
    CylindricalManifold<dim> manifold_0( direction, Point<dim>(0.3 * s, lower + (0.1 * s), 0.0));
    tria.set_manifold(0, manifold_0);

    CylindricalManifold<dim> manifold_1( direction, Point<dim>(0.7 * s, lower + (0.1 * s), 0.0));
    ...
    const unsigned int front_face_inc = pos == 0        ? dirichlet : ((pos - 1) * 3) + 0 + 2;
    const unsigned int back_face      = pos == last - 1 ? robin     : ((pos + 1) * 3) + 0 + 2;
  ...
  void magic2(
    ...
    { // left hole: (manifold id = 0)
      Triangulation<dim> tmp_tria;
      hyper_cube_with_cylindrical_structure(tmp_tria, Point<3>(0.0, lower + (0.1 * s), 0.0), 0.07 * s, 0.1 * s, L, 0);
      GridGenerator::merge_triangulations(tria, tmp_tria, tria, 1e-10, true);
    }
    ...
    { // right hole: (manifold id = 1)
      Triangulation<dim> tmp_tria;
      hyper_cube_with_cylindrical_structure(tmp_tria, Point<3>(0.8 * s, lower + (0.1 * s), 0.0), 0.07 * s, 0.1 * s, L, 1);
      GridGenerator::merge_triangulations(tria, tmp_tria, tria, 1e-10, true);
    }
    ...
    // set the manifold id:
    Tensor<1, dim> direction({0.0, 0.0, 1.0});
    CylindricalManifold<dim> manifold_0( direction, Point<dim>(0.1 * s, lower + (0.2 * s), 0.0));
    tria.set_manifold(0, manifold_0);

    CylindricalManifold<dim> manifold_1( direction, Point<dim>(0.9 * s, lower + (0.2 * s), 0.0));
    tria.set_manifold(1, manifold_1);
    ...
    const unsigned int front_face_inc = pos == 0        ? dirichlet : ((pos - 1) * 3) + 1 + 2;
    const unsigned int back_face      = pos == last - 1 ? robin     : ((pos + 1) * 3) + 1 + 2;
 ...
 template<int dim>
  void magic3(
    ...
    { // left hole: (manifold id = 0)
      Triangulation<dim> tmp_tria;
      hyper_cube_with_cylindrical_structure(tmp_tria, Point<3>(0.2 * s, lower + (0.1 * s), 0.0), 0.07 * s, 0.1 * s, L, 0);
      GridGenerator::merge_triangulations(tria, tmp_tria, tria, 1e-10, true);
    }
    ...
    { // right hole: (manifold id = 1)
      Triangulation<dim> tmp_tria;
      hyper_cube_with_cylindrical_structure(tmp_tria, Point<3>(0.6 * s, lower + (0.1 * s), 0.0), 0.07 * s, 0.1 * s, L, 1);
      GridGenerator::merge_triangulations(tria, tmp_tria, tria, 1e-10, true);
    }
    ...
    // set the manifold id:
    Tensor<1, dim> direction({0.0, 0.0, 1.0});
    CylindricalManifold<dim> manifold_0( direction, Point<dim>(0.3 * s, lower + (0.2 * s), 0.0));
    tria.set_manifold(0, manifold_0);

    CylindricalManifold<dim> manifold_1( direction, Point<dim>(0.7 * s, lower + (0.2 * s), 0.0));
    tria.set_manifold(1, manifold_1);
      ...
      if (cell->material_id() == 0) {
        if ( in_trapez( cell->center(), s ) ) {
          cell->set_material_id( 2 );
                                                                    ...
                                                                    // slize        + neighbour in slize + "+2" shift
    const unsigned int front_face_inc = pos == 0        ? dirichlet : ((pos - 1) * 3) + 2 + 2;
    const unsigned int back_face      = pos == last - 1 ? robin     : ((pos + 1) * 3) + 2 + 2;
  ...
  // Everything that is left to do, is to combine magic steps 1 to 3 into the waveguide
  template<int dim>
  void KirasFMGridGenerator<dim>::make_6er_waveguide_parted (
      Triangulation<dim> &tria,
      unsigned int i,
      const unsigned int refinements,
      const unsigned last,
    ...
    switch(i % 3) {
      case 0:
        magic1(tria, 0.1 * scale, 0.0 * scale, scale, refinements, i / 3, last);
        break;

      case 1:
        magic2(tria, 0.1 * scale, 0.3 * scale, scale, refinements, i / 3, last);
        break;

      case 2:
        magic3(tria, 0.1 * scale, 0.7 * scale, scale, refinements, i / 3, last);
        break;
    }
  }

  template<int dim>
  void KirasFMGridGenerator<dim>::make_6er_waveguide (
      Triangulation<dim> &tria,
      unsigned int i,
      const unsigned int refinements,
      const unsigned last,
      const double scale
        ...
        GridGenerator::merge_triangulations({&tria_magic1, &tria_magic2, &tria_magic3}, tria, 1e-10, true);
