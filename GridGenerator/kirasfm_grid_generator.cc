...
namespace KirasFM_Grid_Generator {
  ...
  // help function: Mark a rectangle in the center of the grid with an different material_id
  template<int dim>
  void mark_block (
  ...
  // --- Set boundary id's ---
  template<int dim>
  void KirasFMGridGenerator<dim>::set_boundary_ids( Triangulation<dim>& in, const int axis) {
        ...
        for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; face++) {
          if (
            ...
            switch( cell->face(face)->boundary_id() - ( 2 * axis) ) {
              ...
              // used to set the boundary data for the analytical test case where the RHS is not zero
              //case 3:
  ...
  template<int dim>
  void KirasFMGridGenerator<dim>::make_simple_waveguide( Triangulation<dim>& in ) {
    ...
    const Point<dim> box_lower_left  = dim == 2 ? Point<dim>(3.0/8.0, 0.0) : Point<dim>(3.0/8.0, 3.0/8.0, 3.0/8.0);
    const Point<dim> box_upper_right = dim == 2 ? Point<dim>(5.0/8.0, 1.0) : Point<dim>(5.0/8.0, 5.0/8.0, 5.0/8.0);
...
template<int dim>
void
KirasFMGridGenerator<dim>::make_gallium_laser (
  ...
  Point<3> upper_right_back_corner( ((outer_width) * block_size) + (2.0 * 0.4), end, (depth + 2) * block_size + thin_layer);
  ...
  for (const auto &cell : tria.cell_iterators()) {
	...
	if( center[2] < block_size ) {
	  ...
	  if( round_down % 2 == 0 ) {
	    // wide stepth
 	    if ( center[0] > block_size && center[0] < (outer_width + 0.4) * block_size )
	    ...
	    if ( center[0] > (half + 1.0) * block_size && center[0] < (outer_width + 1 - half) * block_size )
	      if ( center[2] > block_size && center[2] < (depth + 1) * block_size )
	        cell->set_material_id(3);
	  }
    }
