...
namespace material_blocks
{
  ...
  template <int dim>
  void
  mark_boundary(const Triangulation<dim> &triangulation,
                const Point<dim>          bottom_left,
                const Point<dim>          upper_right,
                types::material_id        id)
  {
  ...
  template <int dim>
  void
  mark_side(const Triangulation<dim> &triangulation,
    ...
    for (auto &cell : triangulation.cell_iterators())
      {
        for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
             face++)
          {
            ...
            if (abs(center(axis) - position) < tol &&
      ...
      for (auto &cell : triangulation.cell_iterators())
        {
          for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
            ...
            {
              center = cell->face(face)->center();
              if (abs(center(axis) - p1(axis)) < tol &&
                  cell->face(face)->at_boundary())
                {
                  if (center(other_axis[0]) < p2(other_axis[0]) &&
                      center(other_axis[0]) > p1(other_axis[0]))
                    {
                      if (center(other_axis[1]) < p2(other_axis[1]) &&
                          center(other_axis[1]) > p1(other_axis[1]))
                        {
                          cell->face(face)->set_boundary_id(boundary_id);
                        }
                    }
...
namespace KirasFM_Grid_Generator
{
  ...
  template <int dim>
  void
  general_cell_xz(Triangulation<dim>     &triangulation,
                  ...
                  unsigned int            material_id = 0)
  {
    if (refinements == 0)
    ...
    else
      {
        ...
        const double lower_x_steps =
          (cell[1](0) - cell[0](0)) / (1.0 * real_refinements);
        const double upper_x_steps =
          (cell[3](0) - cell[2](0)) / (1.0 * real_refinements);
        const double z_steps =
          (cell[4](2) - cell[0](2)) / (1.0 * real_refinements);
        ...
        for (unsigned int i = 0; i < real_refinements; i++)
          {
            ...
            for (unsigned int j = 0; j < real_refinements; j++)
              {
                new_cell = {
                  Point<dim>(lower_x_start - lower_x_steps,
                             lower_y,
                             z_start - z_steps),
                  Point<dim>(lower_x_start, lower_y, z_start - z_steps),
                  Point<dim>(upper_x_start - upper_x_steps,
                             upper_y,
                             z_start - z_steps),
                ...
                general_cell_xz(tria[(i * real_refinements) + j],
                                new_cell,
                                refinements - 1,
                                material_id);
                if (z_start - z_steps >= 0.40 && z_start <= 0.60)
                  {
                    for (auto &cell :
                         tria[(i * real_refinements) + j].cell_iterators())
        ...
        GridGenerator::merge_triangulations(
          //{&tria[0], &tria[1], &tria[2],  &tria[3],  &tria[4],  &tria[5],
          //&tria[6],  &tria[7],
          // &tria[8], &tria[9], &tria[10], &tria[11], &tria[12], &tria[13],
          // &tria[14], &tria[15]},
  ...
  template <int dim>
  void
  y_help_center_lower_part(Triangulation<dim> &triangulation,
                           unsigned int        refinements,
                           double              lower_y,
                           double              hight,
                           double              center,
    ...
    if (refinements == 0)
      {
        ...
        cell = {Point<dim>(center - delta, lower_y + hight, lower_z),
    ...
    else
      {
        ...
        for (unsigned int i = 0; i < real_refinements; i++)
          {
            y_help_center_lower_part(tria[i],
                                     refinements - 1,
                                     lower_y,
                                     hight,
                                     center,
        ...
        GridGenerator::merge_triangulations({&tria[0], &tria[1]},
                                            triangulation,
                                            0.0001);
      }
  }
  ...
  template <int dim>
  void
  y_help_center_middle_part(Triangulation<dim> &triangulation,
                            unsigned int        refinements,
                            double              lower_y,
                            double              hight,
                            double              center,
    ...
    if (refinements == 0)
      {
        ...
        cell = {Point<dim>(center - delta, lower_y, lower_z),
    ...
    else
      {
        ...
        for (unsigned int i = 0; i < real_refinements; i++)
          {
            y_help_center_middle_part(tria[i],
                                      refinements - 1,
                                      lower_y,
                                      hight,
                                      center,
  ...
  template <int dim>
  void
  y_help_center_upper_part(Triangulation<dim> &triangulation,
                           unsigned int        refinements,
                           double              lower_y,
                           double              hight,
                           double              center,
    ...
    if (refinements == 0)
      {
        std::vector<Point<dim>>         cell(8);
        std::vector<Triangulation<dim>> tria(2);
        cell = {Point<dim>(left_c, lower_y, lower_z),
                Point<dim>(center, lower_y, lower_z),
        ...
        cell = {Point<dim>(center, lower_y, lower_z),
    ...
    else
      {
        ...
        for (unsigned int i = 0; i < real_refinements; i++)
          {
            y_help_center_upper_part(tria[i],
                                     refinements - 1,
                                     lower_y,
                                     hight,
                                     center,
  ...
  template <int dim>
  void
  y_help_splitter_lower_part(Triangulation<dim> &triangulation,
                             ...
                             const double        hight)
  {
    ...
    // waveguide part
    {
      cell = {Point<dim>(left_x, lower_y, lower_z),
              Point<dim>(center, lower_y, lower_z),
    ...
    // center
    {
      y_help_center_lower_part(
        tria[2], refinements, lower_y, hight, center, delta, upper_z, lower_z);
    }
    // cladding
    {
      cell = {Point<dim>(0.10, lower_y, lower_z),
      ...
      cell = {Point<dim>(right_x, lower_y, lower_z),
    ...
    GridGenerator::merge_triangulations(
      {&tria[0], &tria[1], &tria[2], &tria[3], &tria[4]}, triangulation, 0.001);
  ...
  template <int dim>
  void
  y_help_splitter_middle_part(Triangulation<dim> &triangulation,
                              const unsigned int  refinements,
                              const double        hight)
  {
    // compute some usefull variables
  ...
  template <int dim>
  void
  y_help_splitter_upper_part(Triangulation<dim> &triangulation,
                             const unsigned int  refinements,
                             const double        hight,
                             ...
                             const double        x_start_r = 0.7)
  {
    ...
    // waveguide part
    {
      ...
      cell = {Point<dim>(right_c, lower_y, lower_z),
              Point<dim>(right_x, lower_y, lower_z),
              Point<dim>(right_c + delta, lower_y + hight, lower_z),
              Point<dim>(right_x + delta, lower_y + hight, lower_z),
              Point<dim>(right_c, lower_y, upper_z),
    ...
    // center
    {
      y_help_center_upper_part(tria[2],
                               refinements,
                               lower_y,
                               hight,
                               center,
  ...
  template <int dim>
  void
  y_help_middle_part(Triangulation<dim> &triangulation,
                     const unsigned int  refinements,
                     const double        hight,
                     const unsigned int  number,
                     double              y_start,
                     ...
                     double              x_start_r)
  {
    ...
    GridGenerator::merge_triangulations(
      {&tria[0], &tria[1], &tria[2], &tria[3]}, triangulation, 0.001);
  }

  template <int dim>
  void
  y_help_lower_part(Triangulation<dim> &triangulation,
                    const unsigned int  refinements,
                    const double        hight,
                    const unsigned int  number,
                    double              y_start,
  ...
  template <int dim>
  void
  y_bsp_upper_part(Triangulation<dim> &triangulation,
                   ...
                   bool                last)
  {
    ...
    for (unsigned int i = 0; i < real_refinements; i++)
      {
        y_help_splitter_upper_part(
          tria[i], refinements, hight, i, y_start, x_start_l, x_start_r);
      }
    ...
    switch (refinements)
      {
        ...
        case 3:
          GridGenerator::merge_triangulations({&tria[0],
                                               &tria[1],
                                               &tria[2],
                                               &tria[3],
                                               &tria[4],
    ...
    if (last == false)
      material_blocks::mark_side<dim>(
        triangulation, 1, y_start + 0.2, id + 3, 0.001);
  ...
  template <int dim>
  void
  y_bsp_upper_middle(Triangulation<dim> &triangulation,
                     const unsigned int  id,
                     const unsigned int  refinements)
  {
    unsigned int real_refinements = 0;
  ...
  template <int dim>
  void
  y_bsp_lower_middle(Triangulation<dim> &triangulation,
                     const unsigned int  id,
                     const unsigned int  refinements,
                     double              y_start,
                     double              x_start_l,
    ...
    switch (refinements)
      {
        case 2:
          GridGenerator::merge_triangulations(
            {&tria[0], &tria[1], &tria[2], &tria[3]}, triangulation, 0.0001);
          break;
  ...
  template <int dim>
  void
  y_bsp_lower_part(Triangulation<dim> &triangulation,
                   const unsigned int  id,
                   const unsigned int  refinements,
                   double              y_start,
                   double              x_start_l,
  ...
  template <int dim>
  void
  KirasFMGridGenerator<dim>::y_beamsplitter(Triangulation<dim> &tria)
  {
    double x_left   = 0.4;
    double x_right  = 0.6;
    double y_bottom = 0.0;
    ...
    if (domain_id == 0)
    ...
    else if (domain_id > 0 && domain_id < 6)
      {
        ...
        y_bsp_lower_middle(
          tria, domain_id, refinements, y_bottom, x_left, x_right);
      }
    else if (domain_id == 6)
      {
        y_bsp_upper_middle(tria, 6, refinements);
      }
    else if (domain_id > 6)
      {
        bool last = (domain_id == N_domains - 1) ? true : false;
        y_bsp_upper_part(tria,
                         domain_id,
                         refinements,
