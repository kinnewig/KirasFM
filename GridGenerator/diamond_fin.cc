...
namespace KirasFM_Grid_Generator
{
  ...
  template <int dim>
  void
  KirasFMGridGenerator<dim>::set_material_ids_for_diamond_fin(
    ...
    const double        block_size)
  {
    ...
    { // hat - part 1
      ...
      Point<3> bottom_left(0,
                           (block_size * (base_hight + fin_hight) * scale),
                           start * scale);
      Point<3> upper_right((2.0 * block_size * half_base_width * scale) +
                             (3 * scale),
                           (block_size * (base_hight + fin_hight) * scale) +
                             (1 * scale),
                           end * scale);
    ...
    { // hat - part 2
      Point<3> bottom_left(block_size * half_base_width * scale,
                           (block_size * (base_hight + fin_hight) * scale) +
                             (1 * scale),
                           start * scale);
      Point<3> upper_right((block_size * half_base_width * scale) +
                             (3.0 * scale),
                           (block_size * (base_hight + fin_hight) * scale) +
                             (2.75 * scale),
                           end * scale);
    ...
    { // base
      Point<dim> bottom_left(0, 0, start * scale);
      Point<dim> upper_right((2.0 * block_size * half_base_width * scale) +
                               (3.0 * scale),
    ...
    { // Diamond Fin:
      Point<dim> bottom_left((block_size * half_base_width * scale) +
                               (1.0 * scale),
                             block_size * base_hight * scale,
                             start * scale);
      Point<dim> upper_right((block_size * half_base_width * scale) +
                               (2.0 * scale),
                             (block_size * (base_hight + fin_hight) * scale) +
                               (1.75 * scale),
                             end * scale);
  ...
  template <int dim>
  void
  KirasFMGridGenerator<dim>::make_diamond_fin(
    Triangulation<dim> &tria,
    ...
    const unsigned int  depth)
  {
    ...
    // Refinement
    //  refine_diamond_fin(tria, base_width_half, base_hight, fin_hight, 4.5,
    //  block_size);
    refine_diamond_fin(
      tria, base_width_half, base_hight, fin_hight, 3.0, block_size);
    //  refine_diamond_fin(tria, base_width_half, base_hight, fin_hight, 1.5,
    //  block_size);

    // set material ids
    set_material_ids_for_diamond_fin(
      tria, base_width_half, base_hight, fin_hight, depth, block_size);
  }
    ...
    double              block_size)
  {
    ...
    for (const auto &cell : tria.cell_iterators())
      {
        double distance = std::sqrt(std::pow(cell->center()[0] - center_x, 2) +
                                    std::pow(cell->center()[1] - center_y, 2));
        if (distance < radius)
