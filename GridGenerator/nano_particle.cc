...
namespace KirasFM_Grid_Generator {
  ...
  template <int dim>
  void eighth_ball_embedding_inverse (
    Triangulation<dim>& tria,
    double inner_radius, /* inner radius */
    double outer_radius  /* outer radius */
  ) {
    ...
    GridGenerator::merge_triangulations(tria, center_tria, tria, 1e-3 * outer_radius);

  }

  // === Help Function: Quarter_shell_embedding ===
  template <int dim>
  void eighth_shell_embedding (
    Triangulation<dim>& tria,
    double inner_radius, /* inner radius */
    double outer_radius  /* outer radius */
  ) {
  ...
  void eighth_to_full (
    ...
    unsigned int x_front = 0, x_back = 0, y_front = 0, y_back = 0, z_front = 0, z_back = 0;
    if ( colorize ) {
      x_front = 0;
      x_back  = 1;
      y_front = 2;
  ...
  template<int dim>
  void mark_layers_for_DD_eighth (
    Triangulation<dim> &tria,
    const unsigned int  domain_id
  ) {
  ...
  void KirasFMGridGenerator<dim>::shell_embedding (
    ...
    // We crate from the quarter ball embedding the (qudratic embedding for a complete ball)
    template <int dim>
    void KirasFMGridGenerator<dim>::ball_embedding_half (
      Triangulation<dim> &tria,
      const double        inner_radius, /* inner radius */
      const double        outer_radius, /* outer radius */
      const unsigned int  part,
    ...
    // === Inverse Ball embedding ===
    template <int dim>
    void KirasFMGridGenerator<dim>::ball_embedding_inverse_half (
      Triangulation<dim> &tria,
      const double        outer_radius, /* outer radius */
      const unsigned int  part,
      bool                colorize
    ...
    // We crate from the quarter ball embedding the (qudratic embedding for a complete ball)
    template <int dim>
    void KirasFMGridGenerator<dim>::shell_embedding_half (
      Triangulation<dim> &tria,
      const double        inner_radius, /* inner radius */
      const double        outer_radius,  /* outer radius */
      const unsigned int  part,
    ...
    // We crate from the quarter ball embedding the (qudratic embedding for a complete ball)
    template <int dim>
    void KirasFMGridGenerator<dim>::ball_embedding_eighth (
      Triangulation<dim> &tria,
      const double        inner_radius, /* inner radius */
      const double        outer_radius, /* outer radius */
      const unsigned int  part,
    ...
    // === Inverse Ball embedding ===
    template <int dim>
    void KirasFMGridGenerator<dim>::ball_embedding_inverse_eighth (
      Triangulation<dim> &tria,
      const double        outer_radius, /* outer radius */
      const unsigned int  part,
      bool                colorize
    ...
    // We crate from the quarter ball embedding the (qudratic embedding for a complete ball)
    template <int dim>
    void KirasFMGridGenerator<dim>::shell_embedding_eighth (
      Triangulation<dim> &tria,
      const double        inner_radius, /* inner radius */
      const double        outer_radius,  /* outer radius */
      const unsigned int  part,
    ...
    template<int dim>
    void KirasFMGridGenerator<dim>::create_nano_particle (
      Triangulation<dim> &tria,
      double ball_radius,
    ...
    template<int dim>
    void KirasFMGridGenerator<dim>::create_half_nano_particle (
      Triangulation<dim> &tria,
      double ball_radius,
      std::vector<double> layer_thickness
    ) {
    ...
    template<int dim>
    void KirasFMGridGenerator<dim>::create_eighth_nano_particle (
      Triangulation<dim> &tria,
      double ball_radius,
      std::vector<double> layer_thickness
    ) {
