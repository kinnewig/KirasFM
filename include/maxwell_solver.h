...
namespace KirasFM
{
  ...
  public:
    // constructor
    MaxwellProblem(ParameterReader   &param,
    ...
    MaxwellProblem(const MaxwellProblem &copy);
    ...
    solve();
    void
    print_results(const unsigned int step) const;

    // assemble interface
    void
    update_interface_rhs();
    void
    assemble_interface_rhs();
    ...
    void
    mark_for_refinement_error_estimator();

    void
    prepare_mark_interface_for_refinement();
    void
    apply_mark_interface_for_refinement();
    ...
    // update:
    void
    update_g_out(SurfaceCommunicator<dim> g);
    void
    update_g_in(SurfaceCommunicator<dim> g);

    void
    update_refine(RefinementCommunicator<dim> r);

  private:
    // === internal functions ===
    void
    setup_system(DoFHandler<dim> &dof_handler_local);


    // error estimator
    void
    error_estimator(Vector<float> &error_indicators) const;

    // output
    void
    output_results(const unsigned int step) const;
    ...
    RefinementCommunicator<dim>
      RefinementOperator; /* Refinement over intefaces */
