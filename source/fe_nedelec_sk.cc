...
FE_NedelecSK<dim, spacedim>::FE_NedelecSK(const unsigned int order)
                                 ...
                                 std::vector<bool>(dim, true)))
{
  ...
  for (unsigned int i = 0; i < GeometryInfo<dim>::max_children_per_face; ++i) {
    face_embeddings[i].reinit(
      this->n_dofs_per_face(face_no),
      this->n_dofs_per_face(face_no)
    );
  }

  compute_face_embedding_matrices_nedelec(
    *this,
    face_embeddings,
    0,
    0,
    1.e-15 * std::exp(std::pow(order, 1.075)));
  ...
  switch (dim) {
      ...
      case 2:
        {
          this->interface_constraints.reinit( 2 * this->n_dofs_per_face(face_no), this->n_dofs_per_face(face_no) );
          for (unsigned int i = 0; i < GeometryInfo<2>::max_children_per_face; ++i) {
            for (unsigned int j = 0; j < this->n_dofs_per_face(face_no); ++j) {
              for (unsigned int k = 0; k < this->n_dofs_per_face(face_no); ++k) {
                this->interface_constraints( i * this->n_dofs_per_face(face_no) + j, k) = face_embeddings[i](j, k);
              }
            }
          }
          break;
      ...
      case 3:
        {
          this->interface_constraints.reinit(
            4 * (this->n_dofs_per_face(face_no) - this->degree), this->n_dofs_per_face(face_no)
          );
          unsigned int target_row = 0;
          for ( unsigned int i = 0; i < 2; ++i )
            for ( unsigned int j = this->degree; j < 2 * this->degree; ++j, ++target_row )
              for ( unsigned int k = 0; k < this->n_dofs_per_face(face_no); ++k )
                this->interface_constraints(target_row, k) = face_embeddings[2 * i](j, k);
          for ( unsigned int i = 0; i < 2; ++i )
            for ( unsigned int j = 3 * this->degree;
                  j < GeometryInfo<3>::lines_per_face * this->degree; ++j, ++target_row
                )
              for ( unsigned int k = 0; k < this->n_dofs_per_face(face_no); ++k )
                this->interface_constraints(target_row, k) = face_embeddings[i](j, k);
          for ( unsigned int i = 0; i < 2; ++i )
            for ( unsigned int j = 0; j < 2; ++j )
              for ( unsigned int k = i * this->degree; k < (i + 1) * this->degree; ++k, ++target_row )
                for ( unsigned int l = 0; l < this->n_dofs_per_face(face_no); ++l )
                  this->interface_constraints(target_row, l) = face_embeddings[i + 2 * j](k, l);
          for ( unsigned int i = 0; i < 2; ++i )
            for ( unsigned int j = 0; j < 2; ++j )
              for ( unsigned int k = (i + 2) * this->degree; k < (i + 3) * this->degree; ++k, ++target_row )
                for ( unsigned int l = 0; l < this->n_dofs_per_face(face_no); ++l )
                  this->interface_constraints(target_row, l) = face_embeddings[2 * i + j](k, l);
          for ( unsigned int i = 0; i < GeometryInfo<3>::max_children_per_face; ++i )
            for ( unsigned int j = GeometryInfo<3>::lines_per_face * this->degree;
                  j < this->n_dofs_per_face(face_no); ++j, ++target_row
                )
              for ( unsigned int k = 0; k < this->n_dofs_per_face(face_no); ++k )
                this->interface_constraints(target_row, k) = face_embeddings[i](j, k);
          break;
...
template <int dim, int spacedim>
double
FE_NedelecSK<dim, spacedim>::shape_value(const unsigned int /*i*/,
...
template <int dim, int spacedim>
double
FE_NedelecSK<dim, spacedim>::shape_value_component(
...
template <int dim, int spacedim>
Tensor<1, dim>
FE_NedelecSK<dim, spacedim>::shape_grad(const unsigned int /*i*/,
                                        const Point<dim> & /*p*/) const
{
  Assert(false, (typename FiniteElement<dim, spacedim>::ExcFENotPrimitive()));
  return Tensor<1, dim>();
...
template <int dim, int spacedim>
Tensor<1, dim>
FE_NedelecSK<dim, spacedim>::shape_grad_component(
  const unsigned int /*i*/,
  const Point<dim> & /*p*/,
  const unsigned int /*component*/) const
{
...
template <int dim, int spacedim>
Tensor<2, dim>
FE_NedelecSK<dim, spacedim>::shape_grad_grad(const unsigned int /*i*/,
                                             const Point<dim> & /*p*/) const
{
  Assert(false, (typename FiniteElement<dim, spacedim>::ExcFENotPrimitive()));
  return Tensor<2, dim>();
...
template <int dim, int spacedim>
Tensor<2, dim>
FE_NedelecSK<dim, spacedim>::shape_grad_grad_component(
  const unsigned int /*i*/,
  const Point<dim> & /*p*/,
  const unsigned int /*component*/) const
{
...
FE_NedelecSK<dim, spacedim>::get_data(
...
template <int dim, int spacedim>
void
FE_NedelecSK<dim, spacedim>::fill_edge_values(
              ...
              for (unsigned int m = 0; m < lines_per_cell; ++m)
                {
                  ...
                  // test if the current face has a hanging node
          ...
          if (flags & (update_values | update_gradients | update_hessians))
            {
                ...
                if ( cell->face(f)->at_boundary() == false )
                  if ( cell->neighbor_is_coarser(f) )
                    for ( unsigned int m = 0; m < 4; ++m ) {
                      unsigned int parent_m = GeometryInfo<dim>::face_to_cell_lines ( f, m );

                      unsigned int v0_loc = GeometryInfo<dim>::line_to_cell_vertices(parent_m , 0);
                      unsigned int v1_loc = GeometryInfo<dim>::line_to_cell_vertices(parent_m , 1);

                      unsigned int v0_glob = cell->parent()->vertex_index(v0_loc);
                      unsigned int v1_glob = cell->parent()->vertex_index(v1_loc);
              ...
              std::vector<unsigned int> face_to_fucks = { 2, 2, 4, 4, 0, 0 };
              for ( unsigned int f = 0; f < 6; ++f ) {
                if ( !cell->face(f)->at_boundary() && !cell->face( face_to_fucks[f] )->at_boundary() )
                  if ( !cell->neighbor_is_coarser(f) && !cell->neighbor_is_coarser( face_to_fucks[f] ) )
                    if ( !cell->neighbor(f)->face( face_to_fucks[f] )->at_boundary() )
                    if ( cell->neighbor(f)->neighbor_is_coarser( face_to_fucks[f] ) ) {
                      unsigned int parent_m = GeometryInfo<dim>::face_to_cell_lines (
                          f,
                          0
                        );
                      unsigned int v0_loc = GeometryInfo<dim>::line_to_cell_vertices(parent_m , 0);
                    ...
                    } // fi: line with hanging node

                if ( !cell->face(f)->at_boundary() && !cell->face( face_to_fucks[f] + 1 )->at_boundary() )
                  if ( !cell->neighbor_is_coarser(f) && !cell->neighbor_is_coarser( face_to_fucks[f] + 1 ) )
                    if ( !cell->neighbor(f)->face( face_to_fucks[f] + 1 )->at_boundary() )
                    if ( cell->neighbor(f)->neighbor_is_coarser( face_to_fucks[f] + 1 ) ) {
                      unsigned int parent_m = GeometryInfo<dim>::face_to_cell_lines (
                          f,
                          1
                        );
...
template <int dim, int spacedim>
void
FE_NedelecSK<dim, spacedim>::fill_face_values(
  const typename Triangulation<dim, dim>::cell_iterator &cell,
  const Quadrature<dim> &                                quadrature,
  const InternalData &                                   fe_data) const
{
          ...
          for (unsigned int m = 0; m < faces_per_cell; ++m)
            {
              ...
              if ( fuck_you ) {
                unsigned int current_glob = cell->parent()->vertex_index(GeometryInfo<dim>::face_to_cell_vertices(m, 0));
              ...
              if ( fuck_you ) {
                if (cell->parent()->vertex_index(GeometryInfo<dim>::face_to_cell_vertices(
                      m, vertices_adjacent_on_face[current_max][0])) >
                    cell->parent()->vertex_index(GeometryInfo<dim>::face_to_cell_vertices(
...
template <int dim, int spacedim>
void
FE_NedelecSK<dim, spacedim>::fill_fe_values(
  const typename Triangulation<dim, dim>::cell_iterator &cell,
...
template <int dim, int spacedim>
void
FE_NedelecSK<dim, spacedim>::fill_fe_face_values(
  const typename Triangulation<dim, dim>::cell_iterator &cell,
...
template <int dim, int spacedim>
void
FE_NedelecSK<dim, spacedim>::fill_fe_subface_values(
  const typename Triangulation<dim, dim>::cell_iterator & /*cell*/,
  const unsigned int /*face_no*/,
...
template <int dim, int spacedim>
UpdateFlags
FE_NedelecSK<dim, spacedim>::requires_update_flags(
...
template <int dim, int spacedim>
std::string
FE_NedelecSK<dim, spacedim>::get_name() const
{
...
FE_NedelecSK<dim, spacedim>::clone() const
{
  return std::make_unique<FE_NedelecSK<dim, spacedim>>(*this);
}
...
FE_NedelecSK<dim, spacedim>::get_dpo_vector(const unsigned int degree)
{
...
template <int dim, int spacedim>
unsigned int
FE_NedelecSK<dim, spacedim>::compute_num_dofs(const unsigned int degree) const
{
...
template <int dim, int spacedim>
void
FE_NedelecSK<dim, spacedim>::create_polynomials(const unsigned int degree)
{
...
template <int dim, int spacedim>
void
FE_NedelecSK<dim, spacedim>::compute_face_embedding_matrices_nedelec(
  const FE_NedelecSK <dim, spacedim> &fe,
  FullMatrix<double> (&matrices)[GeometryInfo<dim>::max_children_per_face],
...
//template class FE_NedelecSK<2>;
template class FE_NedelecSK<3>;
