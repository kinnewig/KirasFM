...
namespace KirasFM
{
    ...
    for (unsigned int i = 0; i < help.size() / 2; i++)
      {
        refrectiv_index_list.push_back(
          std::complex<double>(help[2 * i + 0], help[2 * i + 1]));
      }
  ...
  std::complex<double>
  ParameterReader::get_wavenumber(const Point<3> cell_center) const
  {
    std::string sub_sec      = "Physical constants";
    std::string entry_string = "lambda";
    ...
    Point<3> bla(cell_center[0] - center[0],
                 cell_center[1] - center[1],
                 cell_center[2] - center[2]);
    double   distance_center = bla.norm();

    if (distance_center > 0.5)
      return ref_0 * (2 * PI) / omega;

    std::complex<double> refrective_index_local =
      (ref_1 - ref_0) * exp(std::pow(-0.75 - distance_center, 21)) + ref_0;
