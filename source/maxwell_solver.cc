...
void MaxwellProblem<dim>::setup_system() {
  ...
  std::cout << "Number of degrees of freedom on domain " 
            << domain_id
            << ": " 
            << dof_handler.n_dofs()
            << std::endl;
...
template <int dim>
void MaxwellProblem<dim>::setup_system(DoFHandler<dim>& dof_handler_local) {
  timer.enter_subsection ( "Setup dof system" );
  ...
  //DoFTools::extract_locally_relevant_dofs(dof_handler_local, locally_relevant_dofs);

  constraints.clear();
  constraints.reinit(locally_relevant_dofs);

  DoFTools::make_hanging_node_constraints(
    dof_handler_local, constraints
  );
...
void MaxwellProblem<dim>::assemble_system() {
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()) {
    ...
    const std::complex<double> omega     = prm.get_wavenumber( cell->material_id() );
    ...
    for ( const unsigned int i : fe_values.dof_indices() ) {
      ...
      for ( unsigned int j = i; j < dofs_per_cell; j++ ) { // we are using the symmetry of the here
        ...
        std::complex<double> massterm = ( mu_term * curl_part ) - ( ( omega * omega ) * mass_part );
    ...
    for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; face++) {
      ...
      if ( cell->face(face)->boundary_id() == 0 || cell->face(face)->boundary_id() == 1 ) {
        ...
        for ( const unsigned int i : fe_face_values.dof_indices() ) {
          ...
          for ( const unsigned int j : fe_face_values.dof_indices() ) {
            ...
            // Works for Complex and real, if the refractive index has a complex part, we also write something into the
            // diagonal blocks
            if ( block_index_i == block_index_j )
      ...
      if ( cell->face(face)->boundary_id() >= 2) {
        ...
        for ( const unsigned int i : fe_face_values.dof_indices() ) {
          ...
          // we only want to compute this once
          std::vector<Tensor<1, dim>> phi_i( n_face_q_points );
          //std::vector<double>         surf_curl_phi_i( n_face_q_points );
          //std::vector<double>         div_phi_i( n_face_q_points );
          for (unsigned int q_point = 0; q_point < n_face_q_points; q_point++) {
            phi_i[q_point] = 
            ...
            //  CrossProduct::curl( CrossProduct::trace_tangential(fe_face_values[vec[block_index_i]].gradient(i, q_point), normal[q_point]) ) * normal[q_point];

            //div_phi_i[q_point] =
            //  CrossProduct::div( CrossProduct::curl( fe_face_values[vec[block_index_i]].hessian(i, q_point) ) );
          ...
          for ( const unsigned int j : fe_face_values.dof_indices() ) {
            ...
            for (unsigned int q_point = 0; q_point < n_face_q_points; q_point++) {
              ...
              //  fe_face_values[vec[block_index_j]].curl(j, q_point) * normal[q_point];

              //double div_phi_j = 
              //  fe_face_values[vec[block_index_j]].divergence(j, q_point);
              //  //CrossProduct::div(CrossProduct::curl(fe_face_values[vec[block_index_j]].hessian(j, q_point)));
            ...
            if (block_index_i == block_index_j) { // real values: (blocks: (0,0) and (1,1) )
              cell_matrix(i, j) += omega.imag() * robin;
              //cell_matrix(i, j) += beta.real()  * interface_curl; // for GIBC(alpha)
              //cell_matrix(i, j) += alpha.real() * interface_div; // for GIBC(alpha, beta)
            } else if ( block_index_i == 0 && block_index_j == 1) {
              cell_matrix(i, j) -= omega.real() * robin;
              //cell_matrix(i, j) += beta.imag()  * interface_curl; // for GIBC(alpha)
              //cell_matrix(i, j) += alpha.imag()  * interface_div; // for GIBC(alpha, beta)
            } else if (block_index_i == 1 && block_index_j == 0) { // if ( block_index_i == 1 ) 
              cell_matrix(i, j) += omega.real() * robin;
              //cell_matrix(i, j) -= beta.imag() * interface_curl; // for GIBC(alpha, beta)
              //cell_matrix(i, j) -= alpha.imag() * interface_div; // for GIBC(alpha, beta)
            }
  ...
  for( const auto &cell : dof_handler.active_cell_iterators() ) {
    ...
    for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; face++) {
      ...
      for (unsigned int q_point = 0; q_point < n_face_q_points; q_point++){
        ...
        update_value[q_point] += ( trace(E_curl_re[q_point], normal[q_point])
                        + (imag_i * trace(E_curl_im[q_point], normal[q_point])) );

        // for GIBC(alpha):
        //update_curl[q_point] = beta  * ( CrossProduct::curl(CrossProduct::trace_tangential(E_grad_re[q_point], normal[q_point])) * normal[q_point] 
        //                + (imag_i * CrossProduct::curl(CrossProduct::trace_tangential(E_grad_im[q_point], normal[q_point])) * normal[q_point]) );
      }
  ...
  for(const auto &cell : dof_handler.active_cell_iterators()) {
    ...
    for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; face++) {
      ...
      for ( const unsigned int i : fe_face_values.dof_indices() ) {
        ...
        for (unsigned int q_point = 0; q_point < n_face_q_points; q_point++) {
          update_value[q_point] = SurfaceOperator.value(face_id - 2, domain_id)[face_n[face_id - 2]][q_point];
                                //+ S_curl.value(face_id - 2, domain_id)[face_n[face_id - 2]][q_point];

          //if(first_rhs) {
          //  update_value[q_point] =  2.0 * SurfaceOperator.value(face_id - 2, domain_id)[face_n[face_id - 2]][q_point]; 
          // // update_curl[q_point]  =  2.0 * SurfaceOperator.curl(face_id - 2, domain_id)[face_n[face_id - 2]][q_point]; 
          //} else {
          //  update_value[q_point] = 2.0 * SurfaceOperator.value(face_id - 2, domain_id)[face_n[face_id - 2]][q_point]
          //                        + g_out.value(face_id - 2, domain_id)[face_n[face_id - 2]][q_point];

          //  //update_curl[q_point]  = 2.0 * SurfaceOperator.curl(face_id - 2, domain_id)[face_n[face_id - 2]][q_point]
          //  //                      - g_out.curl(face_id - 2, domain_id)[face_n[face_id - 2]][q_point];
          //}
          ...
          //  fe_face_values[vec[block_index_i]].curl(i, q_point) * normal[q_point];

          std::complex<double> s_tmp = 
            //( (update_value[q_point] * phi_tangential_i) - (update_curl[q_point] * surf_curl_phi_i) ) * fe_face_values.JxW(q_point);
...
// Interpolate the old solution to the new grid (since the solution is needed for the DDM)
template<int dim>
void MaxwellProblem<dim>::refine() {
...
// Interpolate the old solution to the new grid (since the solution is needed for the DDM)
template<int dim>
void MaxwellProblem<dim>::interpolate() {
	pcout << "Start interpolation ... \t";
...
// Interpolate the old solution to the new grid (since the solution is needed for the DDM)
template<int dim>
void MaxwellProblem<dim>::interpolate_global() {
	pcout << "Start interpolation ... \t";
...
// === errpr estimator === 
template<int dim>
void MaxwellProblem<dim>::error_estimator(Vector<float> &error_indicators) const {
  ...
  // cell:
    ...
    //std::vector<Tensor<1,dim>> s_value_re(n_q_points), s_value_im(n_q_points);
    // cell divs:
    std::vector<double> E_div_re(n_q_points), E_div_im(n_q_points);
    // cell hessians:
    std::vector<Tensor<3,dim>> E_hessian_re(n_q_points), E_hessian_im(n_q_points);
  // face:
    // face values:
    std::vector<Tensor<1,dim>> E_face_value_re(n_face_q_points), E_face_value_im(n_face_q_points);
    std::vector<Tensor<1,dim>> s_face_value_re(n_face_q_points), s_face_value_im(n_face_q_points);
    // face curls:
    std::vector<Tensor<1, curl_dim>> E_face_curl_re(n_face_q_points), E_face_curl_im(n_face_q_points);
  // neighbor face
    // neighbor face values:
    std::vector<Tensor<1,dim>> neighbor_value_re(n_face_q_points), neighbor_value_im(n_face_q_points);
    // neighbor face curls:
    std::vector<Tensor<1, curl_dim>> neighbor_curl_re(n_face_q_points), neighbor_curl_im(n_face_q_points);
    // neighbour rhs values:
    std::vector<Tensor<1,dim>> neighbor_rhs_re(n_face_q_points), neighbor_rhs_im(n_face_q_points);
  ...
  for(auto &cell : dof_handler.active_cell_iterators()) {
    ...
    // eta^2_{R,K} = h^2_K/p^2 ( ||curl(mu^{-1} * curl(E_h)) - epsilon * omega^2 E_h - s ||^2 + || div(epsilon * omega^2 E_h) ||^2 )
    for( unsigned int q_point = 0; q_point < n_q_points; q_point++) {
      auto E_curl_curl  = CrossProduct::curl_curl(E_hessian_re[q_point]) + imag_i * CrossProduct::curl_curl(E_hessian_im[q_point]); 
      auto E_value      = E_value_re[q_point] + imag_i * E_value_im[q_point];
      //auto s_value      = s_value_re[q_point] + imag_i * s_value_im[q_point];      // with right hand side
      //auto tmp_1 = s_value - ( mu_term * E_curl_curl ) + ( kappa_term * E_value ); // with right hand side
      auto tmp_1 = - ( mu_term * E_curl_curl ) + ( kappa_term * E_value );
      eta_RK += std::abs(tmp_1 * tmp_1) * fe_values.JxW(q_point);
      auto tmp_2 = kappa_term * (E_div_re[q_point] + imag_i * E_div_im[q_point]);
      eta_RK += std::abs(tmp_2 * tmp_2) * fe_values.JxW(q_point);
    }

    // compute the face based term:
    // eta^2_{J.K} = 1/2 Sum_{faces} ( h_f/p || [ Trace( mu^{-1} * curl(E_h) ) ] ||^2 + || [ n_f * epsilon \omega \trace(E_h) + \trace(s) ] ||^2 )
    for ( unsigned int face_no = 0; face_no < GeometryInfo<dim>::faces_per_cell; ++face_no ) {
      /*
       * First, if this face is part of the boundary, then there is nothing to do. 
       * However, to make things easier when summing up the contributions of the 
       * faces of cells, we enter this face into the list of faces with a zero 
      ...
      /*
       * First, if the neighboring cell is on the same level as this one, i.e. 
       * neither further refined not coarser, then the one with the lower index 
       * within this level does the work. In other words: if the other one has 
       * a lower index, then skip work on this face:
       */
      if (
      ...
      /*
       * Likewise, we always work from the coarser cell if this and its neighbor 
       * differ in refinement. Thus, if the neighboring cell is less refined than 
       * the present one, then do nothing since we integrate over the subfaces 
       * when we visit the coarse cell.
       */
      ...
      // get the normal:
        std::vector<Tensor<1, dim>> normal(n_face_q_points);          
        for (unsigned int q_point = 0; q_point < n_face_q_points; q_point++) {
          normal[q_point] = fe_face_values.normal_vector(q_point) * fe_face_values.JxW(q_point);
        }
      ...
      if (cell->face(face_no)->has_children() == false) {
        ...
        Assert(
          cell->neighbor(face_no).state() == IteratorState::valid,
          ExcInternalError()
        );
        ...
        fe_face_values[E_re].get_function_values(solution, neighbor_value_re);
        fe_face_values[E_im].get_function_values(solution, neighbor_value_im);

        // face_get the curl of the neighbor
        fe_face_values[E_re].get_function_curls(solution, neighbor_curl_re);
        fe_face_values[E_im].get_function_curls(solution, neighbor_curl_im);

        for (unsigned int q_point = 0; q_point < n_face_q_points; q_point++) {

          auto E_jump_curl = (E_face_curl_re[q_point] + imag_i * E_face_curl_im[q_point]) 
                               - (neighbor_curl_re[q_point] + imag_i * neighbor_curl_im[q_point]);

          auto tmp_1 = mu_term * CrossProduct::trace(E_jump_curl, normal[q_point]);
          eta_JK += (cell->face(face_no)->diameter() / (2.0 * (fe.degree + 1) * std::sqrt(1)) ) * std::abs(tmp_1 * tmp_1) * fe_face_values.JxW(q_point);
        ...
        subface_no < face->n_children(); ++subface_no) {
          ...
          // get the rhs values:
          fe_face_values[E_re].get_function_values(system_rhs, neighbor_rhs_re);
          fe_face_values[E_im].get_function_values(system_rhs, neighbor_rhs_im);
          ...
          for (unsigned int q_point = 0; q_point < n_face_q_points; q_point++) {
            ...
            auto tmp_1 = mu_term * CrossProduct::trace(E_jump_curl, normal[q_point]);
            eta_JK += (cell->face(face_no)->diameter() / (2.0 * (fe.degree + 1)) ) * std::abs(tmp_1 * tmp_1) * fe_face_values.JxW(q_point);

            auto E_jump = (E_face_value_re[q_point] + (imag_i * E_face_value_im[q_point]) - s_face_value_re[q_point] - (imag_i * s_face_value_im[q_point]))
                          - (neighbor_value_re[q_point] + (imag_i * neighbor_value_im[q_point]) - neighbor_value_re[q_point] - (imag_i * neighbor_value_im[q_point]));

            auto tmp_2 = kappa_term * CrossProduct::trace_tangential(E_jump, normal[q_point]);
            eta_JK += (cell->face(face_no)->diameter() / (2.0 * (fe.degree + 1)) ) * std::abs(tmp_2 * tmp_2) * fe_face_values.JxW(q_point);
    ...
    eta_RK *= std::pow( (cell->diameter() / std::sqrt(2)) / (fe.degree + 1.0) , 2);
...
// return functions:
template <int dim>
Triangulation<dim>& MaxwellProblem<dim>::return_triangulation() {
...
template <int dim>
SurfaceCommunicator<dim> MaxwellProblem<dim>::return_g_out() {
  return g_out;
}
